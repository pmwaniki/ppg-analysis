...
import os
import json
from models.cnn.networks import resnet1d,Encoder,wideresnet50
...
details_=f"z{enc_representation_size}_l2{enc_l2_}_temp{enc_temp}_x{enc_output_size}_lr{enc_lr_}_bs{enc_batch_size}"
fs=Fs
...
# classifier=MLP(representation_size=enc_representation_size,num_classes=enc_output_size,dropout=0.02)
classifier=nn.Sequential(nn.ReLU(),nn.Linear(enc_representation_size,enc_output_size))
# classifier=nn.Identity()
model=Encoder(raw_model=model_raw,stft_model=model_stft,representation_size=enc_representation_size,
              fc_layer=classifier)
# model=model_raw
# model.fc=nn.Sequential(nn.Linear(model_raw.out_features,enc_output_size))
...
optimizer=torch.optim.SGD([
    {'params': model.stft_model.parameters(),'lr':enc_lr_stft},
    {'params': model.raw_model.parameters(), 'lr': enc_lr_raw},
    {'params':model.fc.parameters(),'lr':enc_lr_fc},
            ],lr=enc_lr_, momentum=0.99,weight_decay=enc_l2_)
...
torch.save(best_model['params'],enc_weights_file)
...
tuned_parameters = { 'clf__C': [1e-3,1e-2,1e-1,1, 10, 100, 1000],
                     'pca__n_components':[int(enc_representation_size / i) for i in 2**np.arange(4)],
...
joblib.dump(clf,enc_weights_file.replace(".pt",".joblib"))