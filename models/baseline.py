...
import os
import json
from models.cnn.networks import groupNorm,resnet1d, wideresnet50
...
details_=f"z{representation_size}_l2{l2_}_drop{dropout_}_lr{lr_}_weight{pos_weight}"
...
model=Net(raw_model=model_raw,stft_model=model_stft,num_classes=1,representation_size=representation_size,dropout=dropout_).to(device)
optimizer=torch.optim.Adam(params=model.parameters(),lr=lr_ ,weight_decay=l2_,betas=(0.99,0.999))
# optimizer=torch.optim.SGD(params=model.parameters(),lr=0.001 ,weight_decay=5e-2,momentum=0.9)
criterion=nn.BCEWithLogitsLoss(pos_weight=torch.tensor(pos_weight) if pos_weight else None).to(device)
...
torch.save({'best':best_model['params'],
           ...
           os.path.join(data_dir,f"results/weights/baseline_{details_}.pt"))

fig,ax=plt.subplots(1)
ax.plot([train for train, test in losses],label="train")
ax.plot([test for train, test in losses],label="test")
plt.legend()
plt.savefig("/home/pmwaniki/Dropbox/tmp/baseline_%s__lr%.5f_l2%.5f_drop%.2f.png" % (os.uname()[1],lr_,l2_,dropout_))
if display:
    plt.show()
else:
    plt.close()
...
save_table3(model="Supervised",precision=precision,recall=recall,specificity=specificity,
