...
enc_distance="DotProduct" #LpDistance Dotproduct
# enc_output_size=64
# enc_batch_size=64
# enc_temp = 0.05
...
# # partial(permute,n_segments=5,p=0.5),
# ]
#
#
#
...
def accuracy_fun(xis_embeddings,xjs_embeddings):
...
def train(model,optimizer,criterion,device,train_loader,val_loader):
    train_loss = 0
...
class Trainer(tune.Trainable):
    ...
    def step(self):
        loss,accuracy=train(self.model,self.optimizer,self.criterion,
...
configs = {
    ...
    'enc_output_size':tune.choice([16,32,64,128,256]),
    'representation_size':tune.choice([enc_representation_size,]),
...
if __name__=="__main__":
    ...
    result = tune.run(
        ...
        num_samples=500,
        name=f"Contrastive-{enc_distance}{enc_representation_size}",
        # resume=False,
        scheduler=scheduler,
